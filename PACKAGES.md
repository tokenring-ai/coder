...
| @tokenring-ai/s3 | 0.1.0 | AWS S3 integration implementing FileSystemProvider and CDNResource for bucket-based file ops and content delivery. | @tokenring-ai/agent, @aws-sdk/client-s3; Peer: @tokenring-ai/cdn, @tokenring-ai/filesystem | S3FileSystemProvider, S3CDNResource | Storage/CDN backend; integrates with agent filesystem and cdn services. |
...
| @tokenring-ai/mysql | 0.1.0 | MySQL database integration extending DatabaseResource for connection pooling, SQL execution, and schema inspection. | @tokenring-ai/database, mysql2 | MySQLResource (alias MySQLService), executeSql, showSchema | DB backend; integrates with agent database service for queries/schemas. |
...
| @tokenring-ai/websearch | 0.1.0 | Abstract web search interface with pluggable providers, tools for search/news/page fetch, and chat commands. | @tokenring-ai/agent, zod | WebSearchProvider (abstract), WebSearchService, tools (searchWeb, searchNews, fetchPage), /websearch command | Web integration base; extended by @tokenring-ai/serper, @tokenring-ai/scraperapi. |
...
| @tokenring-ai/repo-map | 0.1.0 | Generates repository maps by parsing JS/Python/C++ files with Tree-sitter for symbols, yields as agent memories. | @tokenring-ai/agent, @tokenring-ai/filesystem, tree-sitter* | RepoMapService, RepoMapResource, /repo-map command | Codebase awareness; integrates with agent memories and filesystem for parsing. |
...
| @tokenring-ai/memory | 0.1.0 | Ephemeral memory/attention storage for agents, with tools/commands for adding/retrieving items. | @tokenring-ai/agent, @tokenring-ai/utility, zod | MemoryService (abstract), EphemeralMemoryService, tools (add-memory/add-goal/add-focus), /memory /attention commands | Agent context; integrates with chatRequestBuilder for memory injection. |
...
| @tokenring-ai/database | 0.1.0 | Abstract database layer with resource management, SQL execution, schema inspection tools. | @tokenring-ai/agent | DatabaseService, DatabaseResource (abstract), listDatabases/executeSql/showSchema tools | DB abstraction; extended by mysql/sqlite; integrates with agent tools. |
| @tokenring-ai/codebase | 0.1.0 | Service for injecting codebase structure/files into agent context via memories and resources. | @tokenring-ai/agent, @tokenring-ai/filesystem, @tokenring-ai/utility | CodeBaseService, FileTreeResource, WholeFileResource, /codebaseResources command | Code awareness; integrates with agent memories and filesystem. |
...
| @tokenring-ai/chrome | 0.1.0 | Puppeteer tool for browser automation scripts, capturing logs and results. | @tokenring-ai/registry, @tokenring-ai/chat, puppeteer | runPuppeteerScript tool | Web automation; integrates with agent tools for scripting. |
...
- **@tokenring-ai/memory**: Agent memory and attention storage.
...
- **@tokenring-ai/repo-map**: Repository symbol mapping.
...
- **Search/Integrations**: `@tokenring-ai/websearch` (with `@tokenring-ai/serper` or `@tokenring-ai/scraperapi`) feeds results to agent; `@tokenring-ai/file-index` searches codebase internally.
- **Infra/Storage**: `@tokenring-ai/sandbox` (via `@tokenring-ai/docker`) for isolated exec; `@tokenring-ai/database` (with `@tokenring-ai/mysql`) for queries; `@tokenring-ai/aws` for cloud ops.
